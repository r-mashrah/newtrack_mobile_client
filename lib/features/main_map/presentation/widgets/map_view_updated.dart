...
class _MapViewState extends ConsumerState<MapView> {
  ...
  /// بناء المسارات (Trails)
  Set<Polyline> _buildTrails(List<DeviceEntity> devices, bool showTrail) {
    if (!showTrail) return {};

    final trails = <Polyline>{};
    for (final device in devices) {
      if (device.locationHistory.isNotEmpty) {
        final points = device.locationHistory
        ...
        if (points.isNotEmpty) {
          trails.add(
            Polyline(
              polylineId: PolylineId('trail_${device.id}'),
              points: points,
              color: _getTrailColor(device.displayStatus),
              width: 3,
              geodesic: true,
            ),
          );
  ...
  /// الحصول على لون المسار حسب الحالة
  Color _getTrailColor(String status) {
    switch (status) {
      case 'moving':
  ...
  Set<Circle> _buildGeofences(List<DeviceEntity> devices, bool showGeofences) {
    ...
    for (final device in devices) {
      // يمكن إضافة منطق لجلب الأسوار الجغرافية من البيانات
      // هنا مثال توضيحي
      geofences.add(
        Circle(
          circleId: CircleId('geofence_${device.id}'),
          center: LatLng(
            device.lastLocation.latitude,
            device.lastLocation.longitude,
          ),
          radius: 500, // نصف قطر 500 متر
      ...
      builder: (context, ref, child) {
        ...
        return devicesAsync.when(
          ...
          loaded: (devices, isRefreshing, filterQuery, statusFilter) {
            ...
            // تحديث المسارات
            _polylines.clear();
            _polylines.addAll(_buildTrails(devices, mapStyle.showTrail));
